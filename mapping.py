...
entso_df = entso_df.drop(['unit_capacity_mw', 'plant_capacity_mw', 'unit_fuel_platts_entsoe', 'note'], axis=1)

print(entso_df.info())

country_intersect_platts = find_intersection(platts_df['COUNTRY'].values, entso_df['country_platts'].values)

platts_df = platts_df.drop(['SUBREGION', 'AREA', 'STATE', 'CITY', 'YEAR', 'UNIT_CAPACITY_MW'], axis=1)
...
output_df = entso_df.merge(platts_df, left_on=['plant_name', 'plant_primary_fuel_gppd', 'country'], right_on=['PLANT', 'UNIT_FUEL', 'COUNTRY'])

#output_df = output_df.query("unit_name == UNIT")
output_df['unit_match'] = output_df.apply(lambda row: max(row.unit_name.split(), key=len) in row.UNIT and row.unit_name[-1] == row.UNIT[-1], axis=1)
